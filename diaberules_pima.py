...
def run_paper_style(X, y, feature_names):
    ...
    init_rules = []
    global_shap = np.zeros(X.shape[1])
    ...
    for fi, (tr, _) in enumerate(skf.split(X, y)):
        ...
        # Calculate SHAP values for the current tree over its SMOTE training data
        explainer = shap.TreeExplainer(dt)
        sv_raw = explainer.shap_values(Xt_res[:, sel])
        # Handle different SHAP output formats based on version
        if isinstance(sv_raw, list):
            sv = np.abs(sv_raw[1]).mean(axis=0)
        elif len(sv_raw.shape) == 3:
            sv = np.abs(sv_raw[:, :, 1]).mean(axis=0)
        else:
            sv = np.abs(sv_raw).mean(axis=0)
            
        for idx, f_idx in enumerate(sel):
            global_shap[f_idx] += sv[idx]
    ...
    return final, default_class, global_shap
...
if __name__ == "__main__":
    print("=" * 60)
    print("DiabeRules — Corrected Implementation (v7 + SMOTE + SHAP)")
    print("=" * 60)

    final, dc, global_shap = run_paper_style(X, y, feature_names)
    display_rules(final)

    print("\n" + "=" * 60)
    print("Global Feature Importances (SHAP values across all folds)")
    print("=" * 60)
    shap_ranking = sorted([(feature_names[i], global_shap[i]) for i in range(len(feature_names))], key=lambda x: x[1], reverse=True)
    for i, (fname, val) in enumerate(shap_ranking):
        print(f"  {i+1}. {fname: <25} : {val:.4f}")
